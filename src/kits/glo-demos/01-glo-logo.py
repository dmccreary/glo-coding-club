# Lab 01: The GLO Logo
# Streams docs/img/glo-logo-white.png to the 360x360 GC9B72 round display.
#
# The Pico has no PNG decoder, so the image can't be loaded as-is.
# convert_logo.py (run on your computer, not the Pico) flattens the logo
# onto a white background, resizes it to 360x360, and dumps it as raw
# RGB565 pixels -- glo-logo.rgb565, 259,200 bytes -- which is what this
...
# glo-logo.rgb565) to the board's root to have the logo appear a few
# seconds after power-on with no computer attached.
...
with open("glo-logo.rgb565", "rb") as logo:
    y = 0
