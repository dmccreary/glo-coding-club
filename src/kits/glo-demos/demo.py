...
#
# Modes:
#   0  Griffins circle badge   (streamed from griffins-logo.rgb565)
#   1  GLO logo, white background  (streamed from glo-logo.rgb565)
...
# ---------------------------------------------------------------------
# Mode 0 / 1: the logos, streamed a few rows at a time so the whole image
# never has to fit in RAM at once (see 01-glo-logo.py / 02-griffins-logo.py).
...
def draw_griffins_logo():
    stream_logo("griffins-logo.rgb565", BLACK)


def draw_glo_logo():
    stream_logo("glo-logo.rgb565", WHITE)
...
MODES = (
    ("Griffins Badge", draw_griffins_logo, True),
    ("GLO Logo", draw_glo_logo, True),
